...
int main( int argc, char** argv )
{
    ...
    Mat img_ao_mat_lab, img_bo_mat_lab ;
    Mat img_ao_mat_hsv, img_bo_mat_hsv ;
    ...
    //LAB color space
    cvtColor(img_bo_mat,img_bo_mat_lab,COLOR_RGB2Lab);
    cvtColor(img_ao_mat,img_ao_mat_lab,COLOR_RGB2Lab);
    //HSV color space
    cvtColor(img_bo_mat,img_bo_mat_hsv,COLOR_RGB2HSV);
    cvtColor(img_ao_mat,img_ao_mat_hsv,COLOR_RGB2HSV);
    vector<Mat> img_ao_hsv_channels;
    split(img_ao_mat_hsv,img_ao_hsv_channels);
    vector<Mat> img_ao_lab_channels;
    split(img_ao_mat_lab,img_ao_lab_channels);
    //imshow("Ao LAB(L)",img_ao_lab_channels[0]);
    //imshow("Ao LAB(A)",img_ao_lab_channels[1]);
    //imshow("Ao LAB(B)",img_ao_lab_channels[2]);
    imshow("Ao HSV(H)",img_ao_hsv_channels[0]);
    imshow("Ao HSV(S)",img_ao_hsv_channels[1]);
    imshow("Ao HSV(V)",img_ao_hsv_channels[2]);

    vector<Mat> img_bo_hsv_channels;
    split(img_bo_mat_hsv,img_bo_hsv_channels);
    vector<Mat> img_bo_lab_channels;
    split(img_bo_mat_lab,img_bo_lab_channels);
    //imshow("Bo LAB(L)",img_bo_lab_channels[0]);
    //imshow("Bo LAB(A)",img_bo_lab_channels[1]);
    //imshow("Bo LAB(B)",img_bo_lab_channels[2]);
    imshow("Bo HSV(H)",img_bo_hsv_channels[0]);
    imshow("Bo HSV(S)",img_bo_hsv_channels[1]);
    imshow("Bo HSV(V)",img_bo_hsv_channels[2]);
   ...
   for(int iter =0;iter<1;iter++){
                   ...
                   for(int y=roi.y, j=0; j <  roi.height; y++,j++){
                     ...
                     float v=flow_v_mat.at<float>(y,x);
               	     ...
               	     if( x_new >= width_org || x_new <0 || y_new >= height_org || y_new <0){
                                warping_error.at<Vec3b>(y,x)[0]=0;
                                warping_error.at<Vec3b>(y,x)[1]=0;
                                warping_error.at<Vec3b>(y,x)[2]=0;
               	     }
               	     else{
               		 Vec3f diff;
               		 cv::absdiff(img_ao_mat_hsv.at<Vec3b>(y,x),img_bo_mat_hsv.at<Vec3b>(y_new,x_new),diff);
			 diff.val[2]=0;
                         warping_error.at<Vec3b>(y,x)=diff;
                         ...
                         warping_error_norm_linf.at<float>(y,x)=norm(diff,NORM_INF);
               	         //cout<<"current: "<< img_ao_mat_hsv.at<Vec3b>(y,x)<<"\tx:"<<x<<", y:"<<y<<endl;
               	         //cout<<"target : "<< img_bo_mat_hsv.at<Vec3b>(y_new,x_new)<<"\tx_new:"<<x_new<<", y_new:"<<y_new<<endl;
    ...
    //****Warping error map****//
    warping_error_norm_glb=warping_error_norm_linf.clone();
